...
| `elmorlabs` | ElmorLabs PMD2 (USB CDC, VID:PID `0483:5740`)                            | `PCIE1/2/3` on the `--power-max` axis; `HPWR1` (GPU 12VHPWR) and `TOTAL` on a shared high-power axis of `max(100 W, --power-max)` — pass e.g. `--power-max 500` when monitoring a GPU; 10 rails dumped in `--once` |
