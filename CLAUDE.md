...
`mb-powermon.py` is a single-file curses TUI that monitors edge AI NPUs and external power-measurement devices on a workstation. It is adapted from `../envic_ai_python/ai_nvtop.py` with the AMD host probe removed; five probe types are supported: **hailo**, **axelera**, **deepx**, **elmorlabs** (PMD2), and **adafruit** (INA228 over FT232H).
