...
- **MemryX** — MX3 (driver `memx_pcie_ai_chip`, DKMS module `memx_cascade_plus_pcie`) doesn't expose power via hwmon, so the `POW` history stays empty by design (use the INA228 ground-truth path for power). Temperatures come from the standard Linux hwmon node — driver registers `/sys/class/hwmon/hwmonN/` with `name="memx0"` and 16 `tempN_input` files; only the populated slots return values. On the 4-chip MX3-2280-M-4 SKU, `temp1..temp4` populate one per MPU (matching the MPU 0 → MPU 3 dataflow pipeline); on a 2-chip SKU only `temp1..temp2`. The probe autodetects how many MPUs are present and emits a corresponding `T0..T(N-1)` metric per chip. Scanning is hwmon-name-based (`scan_sysfs_memryx()` walks `/sys/class/hwmon` for `name="memx0"` and resolves the BDF via the `device` symlink) — no PCI vendor ID match needed, which is robust to MemryX's vendor ID not being in the public PCI registry. Pure sysfs reads, no SDK or daemon dependency. **Caveat on arch field**: avoid periods in `identity["arch"]` — the snapshot formatter does `str(arch).split(".")[-1]` to unwrap enum qualnames like `BoardType.METIS`, which would also strip "M.2" to "2". MemryXProbe uses `f"M2 ({n}-chip)"` to dodge this.
